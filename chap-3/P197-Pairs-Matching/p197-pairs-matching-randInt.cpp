...
void printMat(vector<vector<NUMTYPE> > mat);
 ...
 */
int _rank(vector<vector<NUMTYPE> > mat, int n){
    NUMTYPE ltmp[100], tmp, a_tmp[100], b_tmp[100];
    int i, j, k;
    int count;
    int all_zero;
...
int maximumMatching(int V, int relation[MAX_V][MAX_V]){
    ...
    uniform_int_distribution<int> dist(1, 1<<20); //整数でやると仕組み的に溢れちゃいそう？
    
    vector<vector<NUMTYPE> > mat;
